  ...
  Future<void> _loadData() async {
    ...
    try {
      final res = await Future.wait([
        _plantRepo.findArchived(),
        _rdwcRepo.getArchivedSystems(),
        _roomRepo.findArchived(),
      ]);
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return PlantryScaffold(
      ...
      bottom: TabBar(
        ...
        tabs: [
          Tab(text: _t['archive_tab_plants']),
          const Tab(text: 'RDWC'),
          Tab(text: _t['archive_tab_rooms']),
        ],
      ),
      body: _isLoading
          ...
          : TabBarView(
              controller: _tabController,
              children: [
                _buildList(_plants, Icons.eco),
                _buildList(_systems, Icons.water_drop),
                _buildList(_rooms, Icons.home),
              ],
            ),
    );
  }
  ...
  Future<void> _confirmRestore(dynamic item) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          '"$itemName" wird wieder aktiviert.',
