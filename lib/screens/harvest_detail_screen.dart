...
'Fehler: $e');
  ...
  Future<void> _showDryWeightDialog() async {
    final TextEditingController dryWeightController = TextEditingController();
    DateTime selectedDryingEndDate = DateTime.now();
          ...
          return AlertDialog(
            title: Row(
              children: [
                ...
                const Text('Trockengewicht erfassen'),
              ],
            ),
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      helperText: 'Endgültiges Gewicht nach Trocknung',
                    ),
                  ...
                  InkWell(
                    onTap: () async {
                      final date = await showDatePicker(
                        context: context,
                        initialDate: selectedDryingEndDate,
                        firstDate: _harvest!.dryingStartDate ?? _harvest!.harvestDate,
                        lastDate: DateTime.now().add(const Duration(days: 7)),
                      );
                      if (date != null) {
                        setDialogState(() {
                          selectedDryingEndDate = date;
                        });
                      }
                    },
                    child: InputDecorator(
                      decoration: InputDecoration(
                        labelText: 'Trocknungsdatum (Ende)',
                      ...
                      child: Text(
                        DateFormat('dd.MM.yyyy').format(selectedDryingEndDate),
                  ...
                  if (dryWeightController.text.isNotEmpty && _harvest!.wetWeight != null)
                    Builder(
                      builder: (context) {
                        final dry = double.tryParse(dryWeightController.text);
                        if (dry == null || dry <= 0 || _harvest!.wetWeight! == 0) {
                          return const SizedBox.shrink();
                        }

                        final loss = ((_harvest!.wetWeight! - dry) / _harvest!.wetWeight!) * 100;
                        final lossGrams = _harvest!.wetWeight! - dry;
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  Text(
                                    '${lossGrams.toStringAsFixed(1)}g Wasser',
            ...
            actions: [
              ...
              ElevatedButton.icon(
                onPressed: () {
                  ...
                  if (dryWeight != null && dryWeight > 0) {
                    Navigator.pop(context, {
                      'dryWeight': dryWeight,
                      'dryingEndDate': selectedDryingEndDate,
                    });
                  } else {
    ...
    if (result != null) {
      try {
        final updated = _harvest!.copyWith(
          dryWeight: result['dryWeight'] as double,
          dryingEndDate: result['dryingEndDate'] as DateTime,
        ...
        if (mounted) {
          AppMessages.showSuccess(context, 'Trockengewicht gespeichert! ✅');
        }
      } catch (e) {
        if (mounted) {
          AppMessages.showError(context, 'Fehler beim Speichern: $e');
        }
      }
    }
  }
  ...
  Future<void> _quickAction(String action) async {
    ...
    switch (action) {
      case 'start_drying':
        updated = _harvest!.copyWith(
          dryingStartDate: DateTime.now(),
          updatedAt: DateTime.now(),
        );
        message = 'Trocknung gestartet! 🌿';
        break;
      case 'end_drying':
        updated = _harvest!.copyWith(
