...
Future<void> _performRecovery(DatabaseExecutor txn) async {
  ...
  // ===========================================
  AppLogger.info('Migration_v19', '🔍 Step 1/5: Checking plants table schema');

  final plantsColumns = await txn.rawQuery('PRAGMA table_info(plants)');
  final columnNames = plantsColumns.map((col) => col['name'] as String).toSet();
  ...
  final missingLogColumns = missingV13LogFields
      .where((col) => !plantLogsColumnNames.contains(col))
      .toSet();
  ...
  try {
    ...
    if (await emergencyDir.exists()) {
      ...
      if (backupFiles.isNotEmpty) {
        ...
        AppLogger.info('Migration_v19', 'Found emergency backup: $backupPath');

        final backupContent = await mostRecentBackup.readAsString();
        backupData = jsonDecode(backupContent) as Map<String, dynamic>;

        AppLogger.info('Migration_v19', '✅ Backup loaded successfully');
      }
    }
    ...
    AppLogger.error('Migration_v19', 'Error searching for backups', e);
  }
  ...
  if (backupData != null && backupData.containsKey('data')) {
    ...
    try {
      ...
      if (plantsBackup != null &&
          plantsBackup.isNotEmpty &&
      ...
      if (plantLogsBackup != null &&
          plantLogsBackup.isNotEmpty &&
          missingLogColumns.isNotEmpty) {
        AppLogger.info(
          'Migration_v19',
          ...
          if (existing.isNotEmpty) {
            ...
            if (recoveredFields.isNotEmpty) {
              ...
              if (recoveredFields.containsKey('nutrient_ppm')) {
                recoveryNote.writeln(
                  '• PPM: ${recoveredFields['nutrient_ppm']}',
                );
              }
              if (recoveredFields.containsKey('light_hours')) {
                recoveryNote.writeln(
                  '• Light: ${recoveredFields['light_hours']}h',
                );
              }
              if (recoveredFields.containsKey('training')) {
                recoveryNote.writeln(
                  '• Training: ${recoveredFields['training']}',
                );
              }
              if (recoveredFields.containsKey('defoliation')) {
                recoveryNote.writeln(
                  '• Defoliation: ${recoveredFields['defoliation']}',
                );
              }
      ...
      AppLogger.error('Migration_v19', 'Data recovery from backup failed', e);
    }
  } else {
    AppLogger.warning(
      'Migration_v19',
...
Future<void> _writeRecoveryStatusFile({
  ...
  try {
    ...
    final statusFile = File(
      path.join(docsDir.path, 'migration_v19_status.json'),
    );
    ...
    AppLogger.error('Migration_v19', 'Failed to write recovery status file', e);
