...
final migrationV42 = Migration(
  version: 42,
  description:
      'RDWC: Add log_status to rdwc_logs, ec_warning_min/max to rdwc_systems',
  up: (db) async {
    ...
    if (!sysColNames.contains('ec_warning_min')) {
      await db.execute(
        'ALTER TABLE rdwc_systems ADD COLUMN ec_warning_min REAL',
      );
    }
    if (!sysColNames.contains('ec_warning_max')) {
      await db.execute(
        'ALTER TABLE rdwc_systems ADD COLUMN ec_warning_max REAL',
      );
    }
  },
  down: (db) async {
