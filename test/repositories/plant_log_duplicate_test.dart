  ...
  group('Fix #4: Duplicate Log Prevention', () {
    ...
    test('findByPlantAndDayNumber() ignores archived logs', () async {
      ...
      expect(
        ...
        isNull,
        reason:
            'Should ignore archived logs (due to UNIQUE constraint WHERE archived = 0)',
      );
    });
    ...
    test('findByPlantAndDayNumber() excludes specific log ID', () async {
      ...
      expect(
        ...
        isNull,
        reason:
            'Should exclude log with matching ID (used for UPDATE validation)',
      );
    });
    ...
    test(
      'save() allows updating existing log (same plant, same day)',
      () async {
        // Arrange: Create plant and log
        final plant = await plantRepo.save(
          Plant(
        ...
        final originalLog = await logRepo.save(
          PlantLog(
            plantId: plant.id!,
            logDate: DateTime(2025, 1, 15),
            dayNumber: 10,
        ...
        final updatedLog = await logRepo.save(
          PlantLog(
    ...
    test(
      'save() allows duplicate day_number if original is archived',
      () async {
        // Arrange: Create plant and archived log
        final plant = await plantRepo.save(
          Plant(
        ...
        final newLog = await logRepo.save(
          PlantLog(
            plantId: plant.id!,
            logDate: DateTime(2025, 1, 15),
