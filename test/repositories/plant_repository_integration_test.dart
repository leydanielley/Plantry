...
void main() {
  ...
  group('PlantRepository - save()', () {
    test('Creating new plant - should insert and return plant with ID', () async {
      ...
      // Assert
      expect(savedPlant.id, isNotNull, reason: 'Saved plant should have an ID');
    ...
    test('Updating existing plant - should update and return modified plant', () async {
      ...
      // Assert
      expect(result.id, equals(savedPlant.id), reason: 'ID should remain the same');
    ...
    test('Updating plant seed date - should recalculate log day numbers', () async {
      ...
      expect(logs.first['day_number'], equals(3), reason: 'Day number should be recalculated from new seed date');
    });

    test('Updating plant seed date to after log date - should delete invalid logs', () async {
      // Arrange - Create plant with seed date and a log
      final plant = Plant(
        name: 'Plant With Logs',
        seedType: SeedType.photo,
      ...
      final logs = await testDb.query('plant_logs', where: 'plant_id = ?', whereArgs: [savedPlant.id]);
      expect(logs.length, equals(0), reason: 'Logs before seed date should be deleted');
    });

    test('Updating plant phase dates - should recalculate phase day numbers', () async {
      ...
      final logs = await testDb.query('plant_logs', where: 'plant_id = ?', whereArgs: [savedPlant.id]);
      expect(logs.length, equals(1));
      expect(logs.first['phase_day_number'], equals(4), reason: 'Phase day number should be recalculated');
    });
  });
  ...
  group('PlantRepository - findAll()', () {
    test('Finding all plants - should return only non-archived plants', () async {
      // Arrange - Create multiple plants
      await repository.save(Plant(name: 'Plant 1', seedType: SeedType.photo, medium: Medium.erde));
      await repository.save(Plant(name: 'Plant 2', seedType: SeedType.auto, medium: Medium.coco));
      await repository.save(Plant(name: 'Archived Plant', seedType: SeedType.photo, medium: Medium.erde, archived: true));

      // Act
      final plants = await repository.findAll();

      // Assert
      expect(plants.length, equals(2), reason: 'Should only return non-archived plants');
    ...
    test('Finding all plants with pagination - should respect limit and offset', () async {
      // Arrange - Create 5 plants
      for (int i = 1; i <= 5; i++) {
        await repository.save(Plant(name: 'Plant $i', seedType: SeedType.photo, medium: Medium.erde));
      }
    ...
    test('Finding all plants when database is empty - should return empty list', () async {
      // Act
      final plants = await repository.findAll();
    ...
    test('Finding all plants should order by ID DESC', () async {
      ...
      final plant1 = await repository.save(Plant(name: 'First', seedType: SeedType.photo, medium: Medium.erde));
      final plant2 = await repository.save(Plant(name: 'Second', seedType: SeedType.photo, medium: Medium.erde));
      final plant3 = await repository.save(Plant(name: 'Third', seedType: SeedType.photo, medium: Medium.erde));
  ...
  group('PlantRepository - findByRoom()', () {
    test('Finding plants by room - should return only plants in that room', () async {
      // Arrange - Seed test data with room
      await TestDatabaseHelper.seedTestData(testDb);

      await repository.save(Plant(name: 'Plant in Room 1', seedType: SeedType.photo, medium: Medium.erde, roomId: 1));
      await repository.save(Plant(name: 'Plant in Room 1 #2', seedType: SeedType.photo, medium: Medium.erde, roomId: 1));
      await repository.save(Plant(name: 'Plant without room', seedType: SeedType.photo, medium: Medium.erde));
    ...
    test('Finding plants by non-existent room - should return empty list', () async {
      // Act
    ...
    test('Finding plants by room - should exclude archived plants', () async {
      ...
      await repository.save(Plant(name: 'Active in Room', seedType: SeedType.photo, medium: Medium.erde, roomId: 1));
      await repository.save(Plant(name: 'Archived in Room', seedType: SeedType.photo, medium: Medium.erde, roomId: 1, archived: true));
  ...
  group('PlantRepository - findByRdwcSystem()', () {
    test('Finding plants by RDWC system - should return plants ordered by bucket number', () async {
    ...
    test('Finding plants by RDWC system - should exclude archived plants', () async {
      // Arrange
      await testDb.insert('rdwc_systems', {
        'id': 1,
        'name': 'Test RDWC System',
      ...
      await repository.save(Plant(name: 'Active', seedType: SeedType.photo, medium: Medium.rdwc, rdwcSystemId: 1, bucketNumber: 1));
      await repository.save(Plant(name: 'Archived', seedType: SeedType.photo, medium: Medium.rdwc, rdwcSystemId: 1, bucketNumber: 2, archived: true));
  ...
  group('PlantRepository - delete()', () {
    test('Deleting existing plant - should remove from database', () async {
      // Arrange
      final plant = Plant(name: 'To Delete', seedType: SeedType.photo, medium: Medium.erde);
      final savedPlant = await repository.save(plant);
    ...
    test('Deleting plant with logs - should cascade delete logs', () async {
      // Arrange
      final plant = Plant(name: 'With Logs', seedType: SeedType.photo, medium: Medium.erde, seedDate: DateTime(2025, 1, 1));
      final savedPlant = await repository.save(plant);
  ...
  group('PlantRepository - archive()', () {
    test('Archiving plant - should set archived flag to 1', () async {
      // Arrange
      final plant = Plant(name: 'To Archive', seedType: SeedType.photo, medium: Medium.erde);
      final savedPlant = await repository.save(plant);
      ...
      final archived = await testDb.query('plants', where: 'id = ?', whereArgs: [savedPlant.id]);
  ...
  group('PlantRepository - count()', () {
    test('Counting plants - should return correct count of non-archived plants', () async {
      // Arrange
      await repository.save(Plant(name: 'Plant 1', seedType: SeedType.photo, medium: Medium.erde));
      await repository.save(Plant(name: 'Plant 2', seedType: SeedType.photo, medium: Medium.erde));
      await repository.save(Plant(name: 'Archived', seedType: SeedType.photo, medium: Medium.erde, archived: true));

      // Act
      final count = await repository.count();

      // Assert
      expect(count, equals(2), reason: 'Should only count non-archived plants');
    });
  ...
  group('PlantRepository - getLogCount()', () {
    test('Getting log count for plant with logs - should return correct count', () async {
      // Arrange
      final plant = Plant(name: 'Plant With Logs', seedType: SeedType.photo, medium: Medium.erde, seedDate: DateTime(2025, 1, 1));
      final savedPlant = await repository.save(plant);
    ...
    test('Getting log count for plant without logs - should return 0', () async {
      // Arrange
      final plant = Plant(name: 'Plant Without Logs', seedType: SeedType.photo, medium: Medium.erde);
      final savedPlant = await repository.save(plant);
  ...
  group('PlantRepository - Error Handling', () {
    test('Finding all plants with database error - should return empty list', () async {
